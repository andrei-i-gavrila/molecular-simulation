...
void echo_running_time() {
    ...
    time_expired = program_finish_time.tv_sec - program_start_time.tv_sec +
                   (program_finish_time.tv_nsec - program_start_time.tv_nsec) * 1e-9;
    time(&time_endtime);
    ...
    char filename[40];
    sprintf(filename, "..\\times\\result_not_optimized_%d.txt", global.N_particles);
    FILE* file = fopen(filename, "w");
    if (file != NULL) {
        fprintf(file, "not optimized for N=%d took %.2lf ms\n", global.N_particles, time_expired);
    }
    fclose(file);
}
